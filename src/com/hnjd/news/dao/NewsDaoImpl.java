...
public class NewsDaoImpl extends BaseDao implements NewsDao {
	...
	//分页查询新闻
	public List<News> getNewsByPage(int currPage,int pageSize) throws Exception {
		String sql = "select * from news where limit ?,?";
		List<News> newsList=new ArrayList<News>();
		ResultSet res= excuteQuery(sql,(currPage-1)*pageSize,pageSize);
		while(res.next()) {
			News news = new News();
			news.setNid(res.getInt("nid"));
			news.setNtid(res.getInt("ntid"));
	...
	//根据条件查询新闻
	public List<News> getNewsByContion(News news) throws Exception{
		String sql="select * from news where ntitle like"
					+"concat('%',?,'%') "
					+"and(adte_format(ncreatDate,'%Y-%m-%d)=date_format ? ,'%Y-%m-%d') "
					+"and ntid=? "
					+"and nauthor like concat('%',?,'%')";
		ResultSet res=excuteQuery(sql,news.getNid(),
	...
	@Override
	public List<News> geutNewsByCondition(News news) throws Exception {
		// TODO 自动生成的方法存根
		return null;
	}
